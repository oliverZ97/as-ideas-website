...
function handleBlogPostsFromFileSystem(blogPostsInPublicDir) {
  ...
  blogPostsInPublicDir.forEach((postFromFs) => {
    // some windows fun
    postFromFs.path = postFromFs.path.replace(new RegExp('\\\\', 'g'), '/');
...
function enrichNewBlogPost(post) {
  ...
  let partsOfBlogPath = post.path.split('/public');
  if (partsOfBlogPath.length < 2) {
    console.error(JSON.stringify(post));
  }
  post.url = partsOfBlogPath[1].replace(".md", "/");
